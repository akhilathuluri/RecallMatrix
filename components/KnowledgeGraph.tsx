...
export function KnowledgeGraph({ open, onOpenChange, userId }: KnowledgeGraphProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[1100px] h-[90vh] glass-card border shadow-2xl rounded-3xl p-0 overflow-hidden flex flex-col">
        <DialogHeader className="px-6 pt-6 pb-3 border-b">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" size="icon" onClick={handleZoomOut} className="rounded-xl h-8 w-8" title="Zoom Out">
                <ZoomOut className="w-4 h-4" />
              </Button>
              <Button variant="outline" size="icon" onClick={handleResetZoom} className="rounded-xl h-8 w-8" title="Reset Zoom">
                <Maximize2 className="w-4 h-4" />
              </Button>
              <Button variant="outline" size="icon" onClick={handleZoomIn} className="rounded-xl h-8 w-8" title="Zoom In">
        ...
        {!loading && nodes.length > 0 && (
          <div className="px-6 py-3 bg-muted/30 border-b flex items-center justify-between text-sm">
        ...
        <div className="flex-1 relative px-6 pb-6 pt-4 overflow-hidden">
          {loading ? (
            <div className="absolute inset-0 flex items-center justify-center bg-background/80 backdrop-blur-sm z-10">
              <div className="text-center">
                <Loader2 className="w-12 h-12 animate-spin mx-auto text-primary mb-4" />
                <p className="text-sm text-muted-foreground">Analyzing your memories...</p>
            ...
            <div className="relative h-full rounded-2xl border-2 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-950 overflow-hidden shadow-inner">
              <canvas
                ref={canvasRef}
                width={1050}
                height={600}
              ...
              {selectedNode && (
                <div className="absolute top-4 right-4 max-w-sm glass-card border-2 rounded-2xl p-5 shadow-2xl animate-in">
                  <div className="flex items-start gap-3 mb-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-bold text-base mb-2 line-clamp-2">{selectedNode.title}</h4>
                      ...
                      <div className="space-y-2 text-xs">
                        ...
                        <div className="flex items-center gap-2 text-muted-foreground">
                          ...
                          <span>{formatDistanceToNow(new Date(selectedNode.created_at), { addSuffix: true })}</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="absolute bottom-4 left-4 glass-card border rounded-xl p-4 text-xs space-y-3 shadow-lg max-w-xs">
